...
EK.main = function(file, model) {
  ...
  # Run Analysis
  if (model == "MM") {
    params = enzKinet2::Michaelis.Menten(EK.data)


  } else if (model == "LCI") {
    params = enzKinet2::LCI(EK.data)


  } else if (model == "TC") {
    params = enzKinet2::Ternary.complex(EK.data)


  } else if (model == "PP") {
    params = enzKinet2::Ping.pong(EK.data)
